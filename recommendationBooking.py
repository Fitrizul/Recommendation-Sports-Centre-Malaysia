...
def important_features(dataset):
    ...
    data["Recommend1"]=data["NAME"]+' '+data["LOCATION"]+' '+data["ADDRESSTEST"] +' '+data["COURTTEST"]
    return data
...
def recommend(title):
    ...
    sorted_scores= sorted_scores[0:18]
    ...
    recommendationBooking_data = pd.DataFrame(columns=['NAME','LOCATION','COURT'])
